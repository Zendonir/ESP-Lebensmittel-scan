...
// index.html als compile-time-Konstante – kein LittleFS-Upload nötig.
...
<div id="cp" class="panel">
  ...
  <div class="add-form">
    ...
    <div class="form-row">
      <label>Kategorie <span class="req">*</span>
...
async function saveWifi() {
  ...
  try {
    ...
    if (r.ok) {
      ...
      const d = await r.json();
      msg.style.color='var(--danger)'; msg.textContent=d.error||'Fehler';
    }
...
function statusOf(d){if(d<0)return'danger';if(d<=3)return'danger';if(d<=7)return'warn';return'ok';}
function daysLabel(d){if(d<0)return'Abgelaufen';if(d===0)return'Heute!';if(d===1)return'1 Tag';return d+' Tage';}
...
async function loadInv(){
  ...
  try{
    ...
    renderInv();
  }catch(e){document.getElementById('invLoading').textContent='Fehler: '+e.message;}
}
...
function renderInv(){
  ...
  items.sort((a,b)=>{
    let va=a[sortKey]!=null?a[sortKey]:'',vb=b[sortKey]!=null?b[sortKey]:'';
...
function renderCP(){
  ...
  const bycat={};
  allCP.forEach((p,idx)=>{
    const c=p.category||'Sonstiges';
    if(!bycat[c])bycat[c]=[];
    bycat[c].push(Object.assign({},p,{_idx:idx}));
  });
...
async function addCP(){
  ...
  const r=await fetch('/api/custom-products',{
    method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({name,brand,barcode,category})
  });
  const data=await r.json();
  if(r.ok){
    msg.style.color='var(--ok)';
    msg.textContent='✓ "'+name+'" ('+category+') hinzugefügt.';
